...
impl Maze {
    pub fn find_neighbors(&self, r: usize, c: usize) -> Vec<(usize,usize)> {
        let mut ns = vec![];
        if r > 1 && self.walls[r-2][c] { ns.push((r-2,c)); }
        if r < self.height-2 && self.walls[r+2][c] { ns.push((r+2,c)); }
        if c > 1 && self.walls[r][c-2] { ns.push((r,c-2)); }
        if c < self.width-2 && self.walls[r][c+2] { ns.push((r,c+2)); }
        ns.shuffle(&mut rand::thread_rng());
        ns
    }
    fn new() -> Maze {
        let width = rand::thread_rng().gen_range(7..12)*2+1;
        let height = rand::thread_rng().gen_range(5..10)*2+1;
        let mut walls: Vec<Vec<bool>> = (0..height).map(|_| (0..width).map(|_| true).collect()).collect();
        let crow = rand::thread_rng().gen_range(0..height/2)*2+1;
        let ccol = rand::thread_rng().gen_range(0..width/2)*2+1;
        let mut track = vec![(crow, ccol)];
        walls[crow][ccol] = false;
        let mut res = Maze { width, height, pos: (1,1), end: (height-2,width-2), walls };
        while let Some(&(crow,ccol)) = track.last() {
            let neighbors = res.find_neighbors(crow, ccol);
            if neighbors.is_empty() { track.pop(); }
            else {
                let (nrow,ncol) = neighbors[0];
                res.walls[nrow][ncol] = false;
                res.walls[(nrow + crow) / 2][(ncol + ccol) / 2] = false;
                track.push((nrow,ncol));
            }
        }
        res
    }
    pub fn mv(&mut self, dp: (isize,isize)) {
        let (dr,dc) = dp;
        let (r,c) = self.pos;
        let (nr,nc) = (r as isize + dr, c as isize + dc);
        if nr >= 0 && nr < self.height as isize && nc >= 0 && nc < self.width as isize && !self.walls[nr as usize][nc as usize] {
            self.pos = (nr as usize, nc as usize);
        }
    }
    fn run_maze(&mut self, screen: &Screen) {
        screen.puts("As you navigate, you find yourself lost in a maze of finely tripped hedges...".to_string());
        screen.puts("You are (*). Use WASD to navigate your way to the end (@).".to_string());
        let (y,x) = screen.pos();
        while self.pos != self.end {
            for l in self.to_string().lines() {
                screen.puts(l.to_string());
            }
            screen.mv(y,x);
            let dp = loop {
                let c = screen.getch();
                if let Input::Character(real_c) = c {
                    match real_c.to_lowercase().next().unwrap() {
                        'w' => break (-1,0),
                        'a' => break (0,-1),
                        's' => break (1,0),
                        'd' => break (0,1),
    ...
    fn to_string(&self) -> String {
        ...
        for r in 0..self.height {
            for c in 0..self.width {
                res.push(if self.walls[r][c] { '#' }
                    else if (r,c) == self.end { '@' }
                    else if (r,c) == self.pos { '*' }
                    else { ' ' });
            }
            res.push('\n');
        }
